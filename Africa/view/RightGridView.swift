...
struct RightGridView: View {
    
    let animal : AnimalListItemModel

    var gridColumns = [GridItem(.fixed((UIScreen.main.bounds.width) / 3), spacing: 0), GridItem(.fixed((UIScreen.main.bounds.width) * 2 / 3), spacing: 0)]
...
struct RightGridView_Previews: PreviewProvider {
    
    static let animal : [AnimalListItemModel] = Bundle.main.decode("animals.json")
    
    static var previews: some View {
        
        RightGridView(animal: animal[0])
    }
}
